...
files = _.takeRight(_.take(fs.readdirSync(DIR), 500), 500)
Promise.each files, (ssFileName, cur) ->
  ...
  .then (ssFile) ->
    ...
    .then ([score1, score2, arena, {player1Deck, player2Deck}]) ->
      ...
      .then ({player1Cards, player2Cards}) ->
        ...
        .then ([deck1, deck2]) ->
          Match.matchExists {
            arena, score1, score2, time, deck1Id: deck1.id, deck2Id: deck2.id
          }
    ...
    .then ->
      unless IS_TEST_RUN
        Promise.promisify(fs.rename) ssFilePath, "#{SS_OUT_DIR}/#{ssFileName}"
.then ->
  console.log 'done'
