...
class ClashRoyalePlayerBaseModel
  ...
  batchUpsertCounterByMatches: (matches) ->
    ...
    mapCondition = ->
      [condition, playerIds, time, gameType,
        crownsEarned, crownsLost] = arguments

      scaledTime = 'DAY-' + moment(time).format 'YYYY-MM-DD'
    ...
    _.forEach matches, (match) ->
      gameType = match.type
      mapCondition(
        'wins', match.winningPlayerIds, match.battleTime, gameType,
        crownsEarned = match.winningCrowns, crownsLost = match.losingCrowns
      )
      mapCondition(
        'losses', match.losingPlayerIds, match.battleTime, gameType,
        crownsEarned = match.losingCrowns, crownsLost = match.winningCrowns
      )
      mapCondition(
        'draws', match.drawPlayerIds, match.battleTime, gameType,
